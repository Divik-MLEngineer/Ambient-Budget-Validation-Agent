...
A2A_RPC_URL = BASE_URL + "/a2a/budget_validation_agent/"
...
def start_server() -> subprocess.Popen[str]:
    ...
    command = [
        ...
        "-m",
        "uvicorn",
        "budget_validation_agent.fast_api_app:app",
        "--host",
        "0.0.0.0",
        "--port",
        "8000",
...
def test_adk_run_sse(server_fixture: subprocess.Popen[str]) -> None:
    ...
    session_response = requests.post(
        f"{BASE_URL}/apps/budget_validation_agent/users/{user_id}/sessions",
        headers=HEADERS,
        json=session_data,
        timeout=60,
    )
    ...
    data = {
        ...
        "new_message": {
            "role": "user",
            "parts": [{"text": json.dumps(purchase_request)}],
        },
        "streaming": True,
    }
...
def test_a2a_chat_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    message = Message(
        ...
        role=Role.user,
        parts=[Part(root=TextPart(text=json.dumps(purchase_request)))],
    )
